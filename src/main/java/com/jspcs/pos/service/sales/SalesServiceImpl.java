...
public class SalesServiceImpl implements ISalesService {
    ...
    public InvoiceResponse createInvoice(CreateInvoiceRequest request, UUID cashierId) {
        ...
        SalesInvoice invoice = SalesInvoice.builder()
                .invoiceNumber("INV-" + System.currentTimeMillis()) // Simple generation, should call sequence in real
                                                                    // DB
        ...
        for (InvoiceItemRequest itemRequest : request.getItems()) {
            ...
            BigDecimal discountPercent = itemRequest.getDiscountPercent() != null ? itemRequest.getDiscountPercent()
                    : BigDecimal.ZERO;
            BigDecimal discountAmount = lineTotal.multiply(discountPercent).divide(BigDecimal.valueOf(100), 2,
        ...
        BigDecimal totalCgst = invoice.getItems().stream().map(InvoiceItem::getCgstAmount).reduce(BigDecimal.ZERO,
                BigDecimal::add);
        BigDecimal totalSgst = invoice.getItems().stream().map(InvoiceItem::getSgstAmount).reduce(BigDecimal.ZERO,
                BigDecimal::add);
